...
bot.on('message', m => { const t = m.toString().trim(); if (t && t.length < 160) console.log('   < ' + t); });
let ran = false;
bot.on('spawn', async () => { if (ran) return; ran = true; await wait(4500);
  for (const c of ['/vev', '/vev version', '/vev about', '/vev reload', '/vev tab groups', '/vev lobby', '/hub', '/veverything version']) {
