...
void main() {
  ...
  group('renderMeter — visible width invariant', () {
    ...
    test('every fraction across the unit range occupies exactly cells columns', () {
  ...
  group('renderMeter — null fraction (missing data)', () {
    test('renders dash repeated cells times, painted faint', () {
      final String rendered = renderMeter(fraction: null, cells: 6, theme: plain);
      expect(rendered, '–' * 6);
    });

    test('painted with the faint tone at truecolor depth', () {
      final String rendered = renderMeter(fraction: null, cells: 4, theme: truecolor);
      expect(rendered, '${truecolor.faint}${'–' * 4}${truecolor.reset}');
    });

    test('never fabricates a zero-fraction bar', () {
      final String nullBar = renderMeter(fraction: null, cells: 6, theme: plain);
  ...
  group('renderMeter — full/track split', () {
    test('0.5 of 10 cells renders 5 full blocks then 5 track cells', () {
      final String rendered = renderMeter(fraction: 0.5, cells: 10, theme: plain);
      expect(rendered, '█' * 5 + '─' * 5);
    });

    test('fraction 0 renders an all-track bar', () {
      final String rendered = renderMeter(fraction: 0.0, cells: 6, theme: plain);
      expect(rendered, '─' * 6);
    });

    test('fraction 1 renders an all-full bar with no track', () {
      final String rendered = renderMeter(fraction: 1.0, cells: 6, theme: plain);
      expect(rendered, '█' * 6);
    });

    test('fraction above 1 clamps to an all-full bar', () {
      final String rendered = renderMeter(fraction: 1.5, cells: 6, theme: plain);
      expect(rendered, '█' * 6);
    });

    test('fraction below 0 clamps to an all-track bar', () {
      final String rendered = renderMeter(fraction: -0.5, cells: 6, theme: plain);
      expect(rendered, '─' * 6);
    });
  });
  ...
  group('renderMeter — eighth-cell partial glyph', () {
    ...
    test('fraction 0.2 with 1 cell yields partial index 1 -> first partial glyph', () {
      // exact = 0.2, full = 0, partialIndex = floor(0.2 * 8) = 1 -> meterPartial[0].
      final String rendered = renderMeter(fraction: 0.2, cells: 1, theme: plain);
      expect(rendered, '▏');
    });
    ...
    test('fraction 0.99 with 1 cell yields the highest partial glyph', () {
      ...
      final String rendered = renderMeter(fraction: 0.99, cells: 1, theme: plain);
      expect(rendered, '▉');
    });
    ...
    test('partial glyph never appears once the bar is already fully ink', () {
      ...
      final String rendered = renderMeter(fraction: 1.0, cells: 3, theme: plain);
    ...
    test('a fractional fill combines full blocks with one partial glyph', () {
      ...
      final String rendered = renderMeter(fraction: 0.55, cells: 4, theme: plain);
      expect(rendered, '██▏─');
    });
  });

  group('renderMeter — tone painting', () {
    test('the entire ink run (full + partial) is painted a single rampTone(ramp, fraction)', () {
      final String rendered = renderMeter(
        fraction: 0.55,
        cells: 4,
        theme: truecolor,
      ...
      expect(rendered, '$tone██▏${truecolor.reset}${truecolor.faint}─${truecolor.reset}');
    });
    ...
    test('defaults to MonitorRamp.load when ramp is omitted', () {
      final String withDefault = renderMeter(fraction: 0.8, cells: 5, theme: truecolor);
